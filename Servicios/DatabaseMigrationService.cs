...
{
    /// <summary>
    /// Gestiona la ejecuci�n autom�tica de scripts de migraci�n de base de datos
    /// durante el proceso de actualizaci�n de la aplicaci�n.
    ///
    /// Convenci�n de nombres de scripts:
    ///   migrate_1.4.0.sql  ?  se ejecuta al actualizar a la versi�n 1.4.0
    ///   migrate_1.5.0.sql  ?  se ejecuta al actualizar a la versi�n 1.5.0
    ///
    /// Los scripts SQL se incluyen en el .zip del GitHub Release dentro de la
    /// carpeta "migrations\". El script .bat de actualizaci�n los ejecuta
    /// autom�ticamente antes de reiniciar la aplicaci�n.
    ///
    /// Cada script debe ser idempotente (puede ejecutarse m�s de una vez sin error).
    /// </summary>
    public class DatabaseMigrationService
    {
        ...
        // Nombre de la tabla que registra qu� migraciones ya fueron aplicadas
        ...
        /// <summary>
        /// Detecta y ejecuta todos los scripts de migraci�n pendientes.
        /// Retorna los resultados de cada migraci�n aplicada.
        /// </summary>
        public async Task<List<MigrationResult>> AplicarMigracionesPendientesAsync()
        {
            ...
            // Obtener scripts pendientes ordenados por versi�n
        ...
        /// <summary>
        /// Retorna los scripts .sql de la carpeta migrations que a�n no fueron aplicados,
        /// ordenados por versi�n ascendente.
        /// </summary>
        private async Task<List<MigrationScript>> ObtenerScriptsPendientesAsync()
        {
            ...
            // Log de diagn�stico: mostrar qu� se descarta y por qu�
        ...
        private async Task<MigrationResult> EjecutarScriptAsync(MigrationScript script)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Registrar como aplicada dentro de la misma transacci�n
...
IF NOT EXISTS (SELECT 1 FROM [{MIGRATIONS_TABLE}] WHERE NombreArchivo = @nombre)
    ...
    /// <summary>
    /// Representa un script de migraci�n con su versi�n parseada del nombre de archivo.
    ...
    public class MigrationScript
    {
        ...
        public MigrationScript(string rutaCompleta)
        {
            ...
            // Parsear versi�n desde "migrate_1.4.0.sql" ? "1.4.0"
    ...
    /// <summary>
    /// Resultado de la ejecuci�n de un script de migraci�n.
    /// </summary>
