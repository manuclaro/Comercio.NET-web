...
{
    /// <summary>
    /// M�dulo de estad�sticas: Compras vs Ventas por Rubro.
    /// Muestra para cada rubro (identificado por el campo Rubro del proveedor)
    /// el total comprado, el total vendido y el porcentaje de ganancia/p�rdida.
    /// </summary>
    public class EstadisticasRubrosForm : Form
    {
        ...
        private void InitializeLayout()
        {
            ...
            // PANEL GR�FICO (barras horizontales simples)
        ...
        private void ActualizarComboDesdeGrilla()
        {
            ...
            // Restaurar selecci�n previa si el rubro sigue en la grilla
        ...
        private async Task ConsultarAsync()
        {
            try
            {
                ...
                using (var conn = new NpgsqlConnection(cs))
                {
                    ...
                    using (var cmd = new NpgsqlCommand(sqlVentas, conn))
                    {
                        cmd.Parameters.AddWithValue("@Desde", dtpDesde.Value.Date);
                        cmd.Parameters.AddWithValue("@Hasta", dtpHasta.Value.Date);
                        if (rubroFiltro != null) cmd.Parameters.AddWithValue("@Rubro", rubroFiltro);
        ...
        // La tabla 'ventas' ya tiene la columna 'rubro' directamente en cada l�nea de venta.
        private string BuildVentasQuery(bool filtrarPorRubro = false)
        {
            string whereRubro = filtrarPorRubro ? "AND LTRIM(RTRIM(rubro)) = @Rubro" : "";
            return $@"
                SELECT
                    COALESCE(NULLIF(TRIM(rubro), ''), 'Sin rubro') AS Rubro,
        ...
        {
            // Formatear columnas num�ricas y colorear seg�n ganancia/p�rdida
